...
在干活当中发现NAS需要一套完整的数据方案，比如硬盘的RAID方式与系统软件的结合，用何种文件系统格式，外部数据如何通信等等。目前来说TX2不是专为此涉及的，再考虑到硬盘组成是非常杂乱的，因此还是考虑将这个设备先将就着用，每块硬盘存各自的数据。过几年可以上NAS之后，将所有数据迁移出去，这里只保存转换后的低码率视频与图片，文档也都是pdf
